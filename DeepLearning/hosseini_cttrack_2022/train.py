...
class Train(object):
    ...
    def __init__(self, args):
        ...
        self.model = None
        self.output_size = 45
        self.gt_kind = self.params.gt_kind
        ...
        self.age_group = self.params.age_group
        if self.trained_model_dir is None:
            self.model_file = join(
                self.model_save_dir,
                f"BCP-CTtrack-{self.gt_kind}-{self.params.grad_directions}-{self.params.age_group}.h5",
            )
        else:
    ...
    def train(self):
        model_trained_on = f"BCP-CTtrack-{self.gt_kind}-{self.params.grad_directions}"  # -{self.params.age_group}'
        wandb.init(
            project="FODNet",
            entity="tobiasforest",
        ...
        x_begin, x_end, y_begin, y_end, z_begin, z_end = 0, 140, 0, 140, 0, 96

        train_data, train_labels, train_mask = process_data(
            # data_dir='/mnt/lrz/data/dHCP_age_new',#'/mnt/lrz/data/dHCP_train/',
            # data_dir='/mnt/lrz/data/dHCP_train',
            data_dir=self.params.data,  # '/mnt/lrz/data/BCP_sampled',
            # dir_list_dir=f'/mnt/lrz/data/dHCP_age_new/{self.params.age_group}_train_50.txt',
            dir_list_dir=self.params.labels,  # f'/mnt/lrz/data/BCP_sampled/BCP_sampled_train_85.txt',
            ...
            y_name=(
                "wm.nii.gz" if self.gt_kind == "MSMT" else "wm_ss3t_20_0_88_1000.nii.gz"
            ),
            mask_name="mask_bet.nii.gz" if "BCP" in self.params.data else "mask.nii.gz",
            b0_name="b0.nii.gz",
        ...
        train_index, valid_index, _, _ = train_test_split(
            self.indices, np.arange(len(self.indices)), test_size=1 - self.split_ratio
        )
        ...
        train_history = self.model.fit(
            my_generator(
                ...
                train_labels,
                self.batch_size,
                num_directions=grad_directions,
            ),
            epochs=self.epochs,
            verbose=2,
            callbacks=callbacks,
