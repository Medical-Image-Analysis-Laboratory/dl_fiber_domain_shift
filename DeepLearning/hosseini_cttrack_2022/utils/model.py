...
class PatchEncoder(layers.Layer):
    def __init__(self, proj_dim):
        ...
        self.projection = layers.Dense(
            units=self.projection_dim - self.positional_embed_dim
        )
        self.position_embedding = layers.Embedding(
            input_dim=self.num_patches,
            output_dim=self.positional_embed_dim,
            input_shape=(self.num_patches, 3),
        )
...
def network(
    ...
    for _ in range(transformer_layers):
        encoded_patches = transformer_block(
            encoded_patches, projection_dim, num_heads, transformer_units, dropout
        )

    attention_map = layers.LayerNormalization(epsilon=1e-6, name="attention_map")(
        encoded_patches
    )
