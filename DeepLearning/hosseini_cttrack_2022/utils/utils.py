...
class DataHandler(object):
    ...
    def load_dwi(self):
        ...
        b_single = np.logical_and(
            self.bvals <= single_shell_bval + 5, self.bvals >= single_shell_bval - 5
        )
    ...
    def get_bm(self):
        b0_mask, brain_mask = median_otsu(
            self.dwi, median_radius=1, numpass=2, vol_idx=[0, 0]
        )
        return brain_mask
    ...
    def mask_data(self):
        ...
        if self.labels_path is not None:
            ...
            self.labels = self.labels * np.tile(
                mask_vol[..., None], (1, 1, 1, labels_vol[-1])
            )
...
def my_generator(indices, X, Y, batch_size, num_directions=6, num_sh=45):
    ...
    while True:
        ...
        for i in range(0, len(indices), batch_size):
            ...
            X_batch = np.empty((actual_batch_size, 3, 3, 3, 6))
