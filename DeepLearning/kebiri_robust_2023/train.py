...
class Train:
    ...
    def read_data(self):
        ...
        self.train_mask, self.val_mask = (
            self.train_mask[: self.train_val_sep],
            self.train_mask[self.train_val_sep :],
        )
    ...
    def train(self):
        ...
        self.model.fit(
            train_generator(
                self.train_data, self.train_labels, self.train_mask, self.batch_size
            ),
            epochs=self.epochs,
            callbacks=self.callbacks,
            validation_data=val_generator_w(
                self.val_data, self.val_labels, self.val_mask, self.batch_size
            ),
            ...
            verbose=2,
            shuffle=True,
            steps_per_epoch=self.train_data.shape[0]
            // self.batch_size
            * self.test_interval,
        )
...
class Test:
    def __init__(
        ...
        n_sig=6,
        n_tar=45,
        output_file_name="dl_mix.nii.gz",
    ):
    ...
    def test(self):
        ...
        for subject in tqdm(self.data_list):
            ...
            dmri_vol = nib.load(os.path.join(subject_dir, self.data_file_name))
            affine = dmri_vol.affine
            dmri = dmri_vol.get_fdata()
            dmri = np.clip(dmri, 0, 1)

            bvecs = np.loadtxt(os.path.join(subject_dir, self.bvec_file_name)).T
            sphere_bvecs = Sphere(xyz=np.asarray(bvecs))
            dmri = sf_to_sh(
                dmri, sphere=sphere_bvecs, sh_order=2, basis_type="tournier07"
            )
            ...
            for lx, ly, lz in product(lx_list, ly_list, lz_list):
                if np.any(
                    ...
                    batch_x = dmri[lx : lx + LX, ly : ly + LY, lz : lz + LZ, :].copy()[
                        np.newaxis, ...
                    ]
            ...
            y_s = np.divide(
                y_s,
                y_c[..., np.newaxis],
                out=np.zeros_like(y_s),
                where=y_c[..., np.newaxis] != 0,
            )
            y_s /= self.mul_coe

            y_s_nii = nib.Nifti1Image(y_s, affine)
            subject_output_dir = os.path.join(self.output_dir, subject)
            nib.save(y_s_nii, os.path.join(subject_output_dir, self.output_file_name))
