...
def process_directory(
    ...
    # Load the nifti file
    x_nii = nib.load(os.path.join(directory, x_name))
    x_data = (
        x_nii.get_fdata()
    )  # dataobj[x_begin:x_end, y_begin:y_end, z_begin:z_end, :]
    if b0_name is not None:
        b0 = nib.load(os.path.join(directory, b0_name))
        b0_data = (
            b0.get_fdata()
        )  # .dataobj[x_begin:x_end, y_begin:y_end, z_begin:z_end]
    ...
    y_nii = nib.load(os.path.join(directory, y_name))
    y_data = (
        y_nii.get_fdata()
    )  # .dataobj[x_begin:x_end, y_begin:y_end, z_begin:z_end, :]
    mask_nii = nib.load(os.path.join(directory, mask_name))
    ...
    del x_data, y_data, mask, b0, y_nii, mask_nii, x_nii
